...
def lista_itens_do_heroi(id_heroi: int) -> List:
    ...
    """
    if not herois.heroi_existe(id_heroi):
        raise HeroiNaoExisteException
    herois_lista = []
    if not itens_do_heroi.heroi_tem_item(id_heroi):
        return herois_lista
...
class TestStringMethods(unittest.TestCase):
    ...
    def test_02a_pega_heroi_invalido(self):
    ...
    def test_10_heroi_tem_item(self):
    ...
    def test_11_heroi_quantos_itens(self):
    ...
    def test_12_itens_do_heroi_acerta_quantidades(self):
    ...
    def test_13_itens_do_heroi_retorna_o_que_foi_pedido(self):
        lista_itens = itens_do_heroi.consulta_itens_por_heroi(1)
        self.assertTrue(type(lista_itens), list)
        item = lista_itens[0]
        self.assertTrue(type(item), dict)
        self.assertIn('idItem', item)
        self.assertIn('idHeroi', item)
        self.assertIn('id', item)
        self.assertEqual(item['idHeroi'], 1)
        item = lista_itens[1]
        self.assertTrue(type(item), dict)
        self.assertIn('idItem', item)
        self.assertIn('idHeroi', item)
    ...
    def test_14_itens_do_heroi(self):
        lista_itens = itens_do_heroi.consulta_itens_por_heroi(3)
        item = lista_itens[0]
        dic_do_item = itens.consultar_item(item['idItem'])
        self.assertEqual(dic_do_item['tipo'], 'varinha')
    ...
    def test_24_dar_item_para_heroi(self):
    ...
    def test_25_heroi_nao_pode_usar_dois_itens_do_mesmo_tipo(self):
