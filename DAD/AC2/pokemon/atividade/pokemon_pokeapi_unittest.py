...
class TestPokeapi(unittest.TestCase):
    ...
    def test_05a_ok(self):
        assert_equals_unordered_list(
            ["grama"], tipos_do_pokemon("chikorita"), self)
        assert_equals_unordered_list(
            ["terra"], tipos_do_pokemon("hippowdon"), self)
        assert_equals_unordered_list(
            ["normal", "fada"], tipos_do_pokemon("jigglypuff"), self)
        assert_equals_unordered_list(
            ["fogo"], tipos_do_pokemon("darumaka"), self)
        assert_equals_unordered_list(
            ["pedra", "voador"], tipos_do_pokemon("archeops"), self)

    @sem_io
    def test_05b_caps(self):
        assert_equals_unordered_list(
            ["voador", "noturno"], tipos_do_pokemon("murKrow"), self)
        assert_equals_unordered_list(
            ["água", "elétrico"], tipos_do_pokemon("cHinChou"), self)
        assert_equals_unordered_list(
            ["lutador", "fantasma"], tipos_do_pokemon("MARSHADOW"), self)
        assert_equals_unordered_list(["aço"], tipos_do_pokemon("KLINK"), self)
        assert_equals_unordered_list(
            ["lutador", "inseto"], tipos_do_pokemon("Heracross"), self)
        assert_equals_unordered_list(
            ["veneno", "noturno"], tipos_do_pokemon("DRAPION"), self)
        assert_equals_unordered_list(
            ["psíquico", "gelo"], tipos_do_pokemon("JYNX"), self)
        assert_equals_unordered_list(
            ["dragão"], tipos_do_pokemon("dRaTiNi"), self)
    ...
    def test_07a_ok_evolucoes_simples(self):
        assert_equals_unordered_list(
            ["charmeleon"], evolucoes_proximas("charmander"), self)
        assert_equals_unordered_list(
            ["combusken"], evolucoes_proximas("torchic"), self)
        assert_equals_unordered_list(
            ["charizard"], evolucoes_proximas("ChArMeLeON"), self)

    @sem_io
    def test_07b_ok_nao_tem_simples(self):
        assert_equals_unordered_list([], evolucoes_proximas("lugia"), self)
        assert_equals_unordered_list(
            [], evolucoes_proximas("turtonator"), self)
    ...
    def test_07c_ok_evolucoes_complexas(self):
        assert_equals_unordered_list(
            ["ninjask", "shedinja"], evolucoes_proximas("nincada"), self)
        assert_equals_unordered_list(["vaporeon", "jolteon", "flareon", "espeon", "umbreon",
                                      "leafeon", "glaceon", "sylveon"], evolucoes_proximas("eevee"), self)
        assert_equals_unordered_list(
            ["hitmonlee", "hitmonchan", "hitmontop"], evolucoes_proximas("tyrogue"), self)
        assert_equals_unordered_list(
            ["poliwhirl"], evolucoes_proximas("Poliwag"), self)
        assert_equals_unordered_list(
            ["gloom"], evolucoes_proximas("oDDiSH"), self)
        assert_equals_unordered_list(
            ["poliwrath", "politoed"], evolucoes_proximas("PoliWHIRL"), self)
        assert_equals_unordered_list(
            ["vileplume", "bellossom"], evolucoes_proximas("GLOOM"), self)
