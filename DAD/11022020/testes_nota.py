...
def resultado_com_pai(frequencia, ac1, ac2, ac3, ac4, ac5, prova, sub, pai1, pai2, pai3, percentil40, percentil60, percentil80):
    mac = 0.3 * ((ac1+ac2+ac3+ac4+ac5) / 5)
    mprova = prova
    if prova < sub:
        mprova=sub
    mprova = 0.4 * mprova
    mpai = sorted([pai1, pai2, pai3]).reverse()
    mpai = 0.3 * ((mpai[0]+mpai[1])/2)
    nota_final = mac + mprova + mpai
    if not frequencia and nota_final <= 6:
        return (nota_final, 'RMF')
    if not frequencia and nota_final > 6:
        return (nota_final, 'RF')
    if nota_final < 6:
        return (nota_final, 'RM')
    return (nota_final, 'AP')
