...
def verifica(agenda, pessoa):
    return True if pessoa in agenda else False
...
{'lucas': {'telefones': [1177788899]},
 'lucas (professor)': {'email': 'lucas.goncalves@faculdadeimpacta.com.br',
                       'telefones': [11999888999, 1177788899]},
...
def conta_telefones(agenda):
    ...
    for value in agenda.values():
        telefones = telefones + len(value['telefones'])
    return telefones
...
def conta_ocorrencias(agenda):
    ...
    for chave in agenda:
        for telefone in agenda[chave]['telefones']:
            if telefone not in ocorrencias:
                ocorrencias[telefone] = 1
            else:
                ocorrencias[telefone] = ocorrencias[telefone]+1
    return ocorrencias
...
for c in vogais:
    ...
    agenda2[c] = {'telefones': [11321321321]}
...
agenda3['fulano'] = {'email': 'fulano@exemplo.com',
...
class TestPartOne(unittest.TestCase):
    ...
    def test_04_conta_letras(self):
        ...
        self.assertEqual(conta_letras('a'*5+'b'*3+'c'*10+'a'),
                         {'a': 6, 'b': 3, 'c': 10})

    def test_05_email(self):
        self.assertEqual(email(agenda_melhor1, 'lucas (professor)'),
                         'lucas.goncalves@faculdadeimpacta.com.br')
        self.assertEqual(email(agenda_melhor1, 'maria'),
                         'maria.aparecida@exemplo.com')
    ...
    def test_06_telefone_principal(self):
        self.assertEqual(telefone_principal(
            agenda_melhor1, 'lucas (professor)'), 11999888999)
        self.assertEqual(telefone_principal(
            agenda_melhor1, 'maria'), 84999777444)
        self.assertEqual(telefone_principal(
            agenda_melhor1, 'lucas'), 1177788899)
    ...
    def test_09_conta_ocorrencias(self):
        ...
        self.assertEqual(conta_ocorrencias(agenda2), {
                         11321321321: 6, 1144440000: 1})
        self.assertEqual(conta_ocorrencias(agenda3), {
