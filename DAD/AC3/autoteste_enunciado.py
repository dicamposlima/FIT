...
def add_answers(questao: dict, answers: list, type: str) -> None:
    for answer in answers[type]:
        if not answer_in_answers(answer, questao[type]):
            questao[type].append(answer)
    index = questoes.index(questao)
    questoes[index].update(questao)
'''
